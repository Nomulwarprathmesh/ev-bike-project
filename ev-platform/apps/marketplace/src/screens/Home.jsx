...
const Home = () => {
  const { data: posters = [], isLoading: postersLoading } = usePosters('home')
                ...
                {posters.map((poster) => {
                  const posterCard = (
                    <motion.div
                      ...
                      className="group relative aspect-[16/6] min-h-40 overflow-hidden rounded-3xl bg-slate-100 shadow-card hover:shadow-card-hover transition-all duration-300"
                    >
                      <img src={poster.image_url} alt={poster.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.02]" />
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950/75 to-transparent p-5">
                        <h2 className="text-lg font-black text-white">{poster.title}</h2>
                        {poster.description && <p className="mt-1 text-sm font-medium text-white/80 line-clamp-2">{poster.description}</p>}
                      </div>
                    </motion.div>
                  )

                  return poster.redirect_url ? (
                    <a key={poster.id} href={poster.redirect_url} className="block" target={poster.redirect_url.startsWith('http') ? '_blank' : undefined} rel="noreferrer">
                      {posterCard}
                    </a>
                  ) : posterCard
                })}
